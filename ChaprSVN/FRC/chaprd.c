...
int send (int port, int sd, struct hostent *hp, char *buffer, int theSize)
{
...
void UDP_send(int sd, struct hostent *hp, char *buffer, int theSize)
{
	...
	int retVal = send(port, sd, hp, buffer, theSize);
	debug_int("retVal:", retVal);
	/*	struct sockaddr_in	dest_addr;
...
void TCP_send(int sd, struct hostent *hp, char *buffer, int theSize)
{
	...
	send(port, sd, hp, buffer, theSize);
